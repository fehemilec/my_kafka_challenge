import logging
from pathlib import Path
from kafka import KafkaConsumer
from kafka.errors import KafkaError


def get_kafka_consumer_client(host, port, certs_path, auth_required=False):

    if auth_required:
        consumer = KafkaConsumer(
            "orders",
            bootstrap_servers=[f"{host}:{port}"],
            security_protocol="SSL",
            ssl_check_hostname=False,
            ssl_cafile=f"{certs_path}/server_certs/CARoot.pem",
            ssl_certfile=f"{certs_path}/server_certs/ca-cert",
            ssl_keyfile=f"{certs_path}/server_certs/ca-key",
            ssl_password="fehemi",
            group_id="my_group",
        )
    else:
        consumer = KafkaConsumer(
            "orders", bootstrap_servers=[f"{host}:{port}"], group_id="my_group"
        )
    return consumer


if __name__ == "__main__":

    abs_path = Path(__file__).parent.parent
    logging.info("Certs Path %s ", abs_path)
    print("Print Certs Path %s ", abs_path)
    try:
        consumer = get_kafka_consumer_client(
            host="kafka-2", port=9095, certs_path=abs_path, auth_required=True
        )

        for message in consumer:
            # decode bytes
            value = message.value.decode("utf-8")
            print(
                "%s:%d:%d: key=%s value=%s"
                % (message.topic, message.partition, message.offset, message.key, value)
            )
            logging.info(
                "Received new order %s:%d:%d key=%s value=%s",
                message.topic,
                message.partition,
                message.offset,
                message.key,
                value,
            )
    except KafkaError as e:
        print("Kafka error: ", e)
        logging.info("Kafka error: %s", e)
